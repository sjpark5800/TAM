    ...
    def __call__(self, vid_name: str) -> Tuple[str, Dict[str, float], Dict[str, float]]:
        ...
        for all_frames, obj_id, gt_path, pred_path in to_evaluate:
            ...
            for frame in all_frames:
                print(f"Evaluating {vid_name} {obj_id} {frame}")
...
class Evaluator:
    ...
    def feed_frame(self, mask: np.ndarray, gt: np.ndarray):
        ...
        for obj_idx in all_objects:
            ...
            # seg = obj_mask.astype(bool)
            # seg[seg > 0] = 1

            # if np.atleast_3d(seg).shape[2] != 1:
            #     print("dddd")




            # seg = obj_gt.astype(bool)
            # seg[seg > 0] = 1
    ...
    for gt_root, mask_root in zip(gt_roots, mask_roots):
        ...
        if single_dataset:
            if verbose:
                ...
                results = []
                for vid in videos:
                    print("Evaluating video:", vid)
                    results.append(evaluator(vid)) 
